...
In your formtastic form, use <tt>:as => :rich</tt> to insert Rich. Like so:
  ...
  <%= semantic_form_for @post do |f| %>
     <%= f.inputs do %>
       <%= f.input :name %>
       <%= f.input :title %>
       <%= f.input :body, :as => :rich, :config => { :defaultStyle => "myCrazyPaperclipStyle" } %>
     <% end %>
     <%= f.buttons do %>
       <%= f.commit_button %>
     <% end %>
...
* upload non-image files (almost there)
* scoping assets to other objects
* drag & drop uploading
* separate form control to select/upload images without CKEditor (almost there)
