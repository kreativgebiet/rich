...
To enable uploads of video files, set the following in your Rich initializer:

  # enable video uploads. default is false.
  config.allow_video_uploads = true
  # restrict the type of files that can be uploaded. defaults to :all for no restrictions.
  config.allowed_video_types = ['application/mp4']

This will enable a new button on your CKEditor toolbar where you can upload video files. Keep in mind that even if an image is uploaded through this method, it will not go through any processing you might have set up for your images.

Video uploads also work from within the file picker when you pass in the <tt>:type => 'file'</tt> option. A Rails Admin example:
